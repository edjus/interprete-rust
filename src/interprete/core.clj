(ns interprete.core
  (:require [interprete.funciones_auxiliares :refer :all]))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Interprete de rust, en realidad interpreta el bycote generado del comando VIRTU.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defn dump [vector]
   (cond
     (nil? vector) (println 0 nil)
      :else (print (apply str (map-indexed (fn [idx, item] (str idx " " item "\n")) vector)))
   )
)
...
;                                                               ^^^^^^^^^^^^     ^  ^^^^^^^^^^^^^^^^^^^^^^^
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn iniciar-contexto-local-aux [contexto]
  (assoc contexto 0 (conj (first contexto) (count (second contexto))))
  )

(defn inicializar-contexto-local [amb]
  (if (= (estado amb) :sin-errores)
    (let [context (contexto amb)]
      (assoc amb 4 (iniciar-contexto-local-aux context))
      )
    amb
    )
  )
...
; nil
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn buscar-terna [sub-context, direc]
  (cond
    (empty? sub-context) nil
    :else (let [h (first sub-context) t (rest sub-context)]
            (cond
              (= (last h) direc) h
              :else (buscar-terna t direc)
              )
      )
    )
  )

(defn tipo-terna [terna-context]
  (nth terna-context 1)
  )

(defn tipo-retorno [tipo-terna]
   (last (second tipo-terna))
  )

(defn buscar-tipo-de-retorno [amb, direc]
  (let [context (contexto amb)]
    (tipo-retorno (tipo-terna (buscar-terna (second context) direc)))
    )
)
...
(defn intentar-parsear [parser, x]
  (try
      (parser x)
    (catch Exception e x))
  )
...
(defn pasar-a-int [x]
  (cond
    (int? x) x
    (string? x) (intentar-parsear read-string x)
    :else (intentar-parsear int x)
    )
  )
...
(defn handle-resultado-string-parse-float [resultado]
  (cond
    (int? resultado) (float resultado)
    :else resultado
    )
  )
(defn pasar-a-float [x]
  (cond
    (float? x) x
    (string? x) (handle-resultado-string-parse-float (intentar-parsear read-string x))
    :else (intentar-parsear float x))
  )
